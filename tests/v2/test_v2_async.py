...
@pytest.mark.asyncio
async def test_tracing_headers(valid_client, fc_server):
    await valid_client.systems()
    await valid_client.partitions("cluster")
...
@pytest.mark.asyncio
async def test_user_provided_correlation_id(valid_client, fc_server):
...
@pytest.mark.asyncio
async def test_exception_carries_tracing_ids(invalid_client, fc_server):
    with pytest.raises(UnexpectedStatusException) as excinfo:
        await invalid_client.systems()
