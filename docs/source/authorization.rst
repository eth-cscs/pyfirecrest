...
What pyFirecREST will need in the end is only a python object that describes how to authenticate a request, in one of two ways:

- a method ``auth_headers()`` returning a dictionary with the HTTP headers that authenticate a request (with the async client this method may also be a coroutine), or
- a method ``get_access_token()`` returning a valid access token, which pyFirecREST sends as ``Authorization: Bearer <token>``.

If both are present, ``auth_headers()`` takes precedence.
...
Some FirecREST deployments authenticate with an API key sent in a dedicated header instead of a bearer token.
For these you can use the ``ApiKeyAuth`` class; by default the key is sent in the ``X-API-Key`` header, but the header name is configurable:

.. code-block:: Python

    import firecrest as f7t

    auth = f7t.ApiKeyAuth(<api_key>)
    # or, if the deployment expects a different header name:
    auth = f7t.ApiKeyAuth(<api_key>, header_name="Api-Key")

    client = f7t.v2.Firecrest(firecrest_url=<url>, authorization=auth)

Keep the API key secret: read it from an environment variable or a secret manager rather than committing it to your repository.

If your deployment needs a combination of headers (for example a bearer token together with an extra header), write your own class with an ``auth_headers()`` method returning all of them.
