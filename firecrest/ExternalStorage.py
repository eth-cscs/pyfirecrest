...
import sys
import time
from typing import Any, ContextManager, Optional, overload, Sequence, Tuple, List, Union
...
class ExternalStorage:
    ...
    @property
    def client(self) -> Firecrest | AsyncFirecrest:
    ...
    def _update(self) -> None:
        if self._status not in self._final_states:
            task = asyncio.run(self._client._task_safe(self._task_id, self._responses))
...
class ExternalDownload(ExternalStorage):
    ...
    def invalidate_object_storage_link(self) -> None:
        ...
        """
        asyncio.run(self._client._invalidate(self._task_id))
